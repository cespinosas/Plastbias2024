...
public:
  ...
  void make_copy(GraphC& vacia);
	void put_subgraph_into(GraphC& nueva, set<int> &cuales, bool withnames);
	void put_subgraph_into(GraphC& nueva, set<int> &cuales);
	void make_subgraph_of(GraphC &templ, set<int> &cuales, bool withnames);
	void make_subgraph_of(GraphC &templ, set<int> &cuales);
	void copy_wdeg(GraphC &templ);
  GraphC(int n, double c, Alea& jacta, bool dirnw);
  void make_nw(int n, double c, bool dirnw);
  GraphC(int n, int e, Alea& jacta, bool dirnw);
  ...
  void force_interaction_undir(int source, int target, double value);
  void change_interaction(int source, int target, double value);
  void change_interaction_undir(int source, int target, double value);
  ...
  void set_directed();
	...
	void transform_to_isomorph(GraphC& vacia, int* vec);
  ...
  //analyses
  double distance_from_nw(GraphC& otra);
  bool equal_adjmat(GraphC& otra);
  ...
  //for evolution
  void build_offspring(GraphC &mother, GraphC &father, bool *frommom);
  void mate(GraphC mother, GraphC father);
